...
def isValidWord(word, hand, wordList):
    ...
    for letter in word:
        if letter not in check_hand.keys() or check_hand[letter] <= 0:
            return False
        check_hand[letter] -= 1
...
def calculateHandlen(hand):
    ...
    returns: integer
    """
    # Fancy code but not good ;p

    #from functools import reduce
    #if len(hand) > 0:
    #    return reduce(lambda x, y: x + y, [x for x in hand.values()])
    #return 0
